...
class FoodViewController: UIViewController {
    ...
    @objc func nextYear(_ sender: UIButton){
        ...
        loadDBModel.loadMonthlyFoodTransition(groupID: groupID, year: nextYear, settlementDay: settlementDay, startDate: startDate, endDate: endDate)
    }
    ...
    @objc func lastYear(_ sender: UIButton){
        ...
        startDate = dateFormatter.date(from: "\(Int(year)! - 1 + yearCount)年\("12")月\(settlementDay)日")!
        endDate = dateFormatter.date(from: "\(Int(year)! + yearCount)年\("12")月\(settlementDay)日")!
        loadDBModel.loadMonthlyFoodTransition(groupID: groupID, year: lastYear, settlementDay: settlementDay, startDate: startDate, endDate: endDate)
    }
